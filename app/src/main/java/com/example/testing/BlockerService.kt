...
class BlockerService : Service() {
    ...
    private val checkRunnable = object : Runnable {
        override fun run() {
            ...
            UsageUtils.clearExpiredBuffers(this@BlockerService) // Clear expired buffers
            checkAndBlockApps()
    ...
    private fun checkAndBlockApps() {
        ...
        try {
            ...
            // IMMEDIATE BLOCKING: If this is a blocked app and time limit is already reached, check for buffer first
            if (blocked.contains(currentApp)) {
                ...
                if (limit > 0) {
                    ...
                    // If already over limit, check for active buffer before blocking
                    if (usedMinutes >= limit) {
                        // Check if user has an active buffer for this app
                        if (UsageUtils.hasActiveBuffer(this, currentApp)) {
                            val bufferRemaining = UsageUtils.getBufferTimeRemaining(this, currentApp)
                            android.util.Log.d("BlockerService", "App over limit but has active buffer: $currentApp, Buffer remaining: ${bufferRemaining}s")
                            // Allow access during buffer time
                            return
                        } else {
                            ...
                            if (!blockingInProgress) {
                                ...
                                // Show buffer math challenge instead of redirecting to home
                                showBufferMathChallenge(currentApp)
                    ...
                    if (newUsedMinutes >= limit) {
                        ...
                        if (!blockingInProgress) {
                            blockingInProgress = true
                            handler.post {
                                Toast.makeText(this, "Time limit reached for $currentApp.", Toast.LENGTH_SHORT).show()
                            }
                            // Show buffer math challenge instead of redirecting to home
                            showBufferMathChallenge(currentApp)
                            // Reset flag after delay
    ...
    private fun showBufferMathChallenge(packageName: String) {
        val appName = getAppName(packageName)
        
        android.util.Log.d("BlockerService", "Attempting to show buffer math challenge for $packageName ($appName)")
        
        // First try to kill the blocked app to ensure our activity shows
        tryKillBlockedApp(packageName)
        
        // Add a small delay to ensure the app is killed before launching our activity
        handler.postDelayed({
            try {
                android.util.Log.d("BlockerService", "About to launch BufferMathChallengeActivity...")
                
                // Show buffer math challenge screen
                val intent = Intent(this, BufferMathChallengeActivity::class.java)
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_NO_ANIMATION)
                ...
                android.util.Log.d("BlockerService", "Successfully launched BufferMathChallengeActivity for $packageName")
                
                // Show a notification to confirm the activity was launched
                handler.post {
                    Toast.makeText(this, "Buffer math challenge launched for $appName", Toast.LENGTH_SHORT).show()
                }
                
            } catch (e: Exception) {
                android.util.Log.e("BlockerService", "Failed to launch BufferMathChallengeActivity: ${e.message}")
                e.printStackTrace()
